...
export default function Map( { stationDelays } ) {
  ...
  const timeDelayCircle = (AdvertisedTime: string | number | Date, EstimatedTime: string | number | Date, centerCords: string[]) => {
  ...
  const listMarker = stationDelays.map((station: { Geometry: { WGS84: string; }; AdvertisedLocationName: string; AdvertisedTimeAtLocation: string; EstimatedTimeAtLocation: string; }, index: React.Key) => {
